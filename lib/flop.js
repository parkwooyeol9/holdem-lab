...
import { FLOP_ORDER, classify } from "./villains";
...
export function alivePositions(preActions, heroPos) {
  return FLOP_ORDER.filter((pos) => pos === heroPos || preActions?.[pos] !== "fold");
}
...
export function legalFlopActions(flopActions, pos, alive) {
  ...
  const before = alive.slice(0, idx).map((p) => flopActions?.[p] || "");
...
export function parseFlopSpot(flopActions, ahead) {
  const incomplete = ahead.some((pos) => !flopActions?.[pos]);
  let aggressor = null;
  let level = 0;
  for (const pos of ahead) {
    const act = flopActions?.[pos];
    if (act === "bet") {
      aggressor = pos;
      level = 1;
    } else if (act === "raise") {
  ...
  return { incomplete, aggressor, level };
}
...
export function recommendFlop({ hole, flop, heroPos, actions, flopActions, seats }) {
  ...
  if (!hand) return emptyRec("Need hole cards", "홀카드 두 장을 먼저 고르세요.");
  if (board.length < 3) return emptyRec(`${hand} · flop`, "플랍 카드 3장을 깔면 추천이 나옵니다.");

  const ahead = flopAhead(heroPos, actions);
  const spot = parseFlopSpot(flopActions, ahead);
  if (spot.incomplete) {
    return emptyRec(`${hand} · flop`, "플랍에서 당신 앞 액션(체크/벳/콜/레이즈/폴드)을 표시하세요.");
  }
